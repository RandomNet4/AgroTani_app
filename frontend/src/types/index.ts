...
// --- Status Enums ---
export type StatusVerifikasi = 'pending' | 'approved' | 'rejected' | 'survey' | 'selesai' | 'selesai_panen' | 'dikirim' | 'proses_timbang';
...
export type StatusPengajuanJual = 'pending' | 'approved' | 'rejected' | 'survey' | 'pickup_dijadwalkan' | 'proses_timbang' | 'dikirim' | 'selesai';
...
export interface PengajuanJual {
  ...
  gudangTujuanNama?: string;
  tipePenjualan?: 'PO_GUDANG' | 'MANDIRI';
}
...
export interface Pembayaran {
  ...
  nomorInvoice: string;
  buktiTransfer?: string;   // Nomor referensi atau base64 foto bukti transfer bank
  buktiTunai?: string;      // base64 atau URL foto kertas bukti pembayaran tunai
  dibuatOleh?: string;
}
