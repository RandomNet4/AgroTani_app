...
const DataLahanAdminPage: React.FC = () => {
  const { lahan: listLahan, petani: listPetani, verifyLahan } = useData();
  ...
  const handleVerifyLahan = async (id: string, status: string, name: string) => {
    const land = listLahan.find(l => l.id === id);
    const petani = land ? listPetani.find(p => p.id === land.petaniId) : null;
  ...
  const filtered = listLahan.filter(l => {
    const petani = listPetani.find(p => p.id === l.petaniId);
  ...
  const selectedLahan = listLahan.find(l => l.id === selectedLahanId);
  const selectedPetani = selectedLahan ? listPetani.find(p => p.id === selectedLahan.petaniId) : null;

  const totalLuas = listLahan.reduce((sum, l) => sum + l.luasHektar, 0);
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Total {listLahan.length} Lahan • {totalLuas.toFixed(1)} Ha</p>
        </div>
