...
const ApprovalTenderPage: React.FC = () => {
  const { tenderPetani: listTenderPetani, tender: listTender, petani: listPetani, verifyTenderPetani } = useData();
  const [filterStatus, setFilterStatus] = useState('semua');
  const [loadingId, setLoadingId] = useState<string | null>(null);

  const filtered = filterStatus === 'semua'
    ? listTenderPetani
    : listTenderPetani.filter(tp => tp.statusApproval === filterStatus);
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="bg-amber-50 rounded-xl px-4 py-2 text-center">
          <p className="text-2xl font-bold text-amber-700">{listTenderPetani.filter(tp => tp.statusApproval === 'pending').length}</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(tp => {
                const tender = listTender.find(t => t.id === tp.tenderId);
                const petani = listPetani.find(p => p.id === tp.petaniId);
