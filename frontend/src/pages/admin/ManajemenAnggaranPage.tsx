...
const ManajemenAnggaranPage: React.FC = () => {
  const { bukuKas: listBukuKas } = useData();
  const [searchTerm, setSearchTerm] = useState('');
  
  const totalAnggaran = listBukuKas
    .filter(b => b.tipeTransaksi === 'Uang Masuk')
    .reduce((s, b) => s + b.nominal, 0);
    
  const totalPencairan = listBukuKas
    .filter(b => b.tipeTransaksi === 'Uang Keluar' && b.kategori === 'Pembayaran Petani')
    .reduce((s, b) => s + b.nominal, 0);
    
  // Saldo akhir adalah saldo pada baris transaksi terakhir
  const saldoAkhir = listBukuKas.length > 0 ? listBukuKas[listBukuKas.length - 1].saldoAkhir : 0;

  const filteredKas = listBukuKas.filter(k => 
