...
const ManajemenJualPanenPage: React.FC = () => {
  const { pengajuanJual: listPengajuanJual, verifyPengajuanJual, komoditas: listKomoditas } = useData();
  ...
  const filtered = filterStatus === 'semua'
    ? listPengajuanJual
    : listPengajuanJual.filter(p => p.status === filterStatus);

  const detail = listPengajuanJual.find(p => p.id === selected);
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="bg-amber-50 rounded-xl px-4 py-2 text-center">
          <p className="text-2xl font-bold text-amber-700">{listPengajuanJual.filter(p => p.status === 'pending').length}</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(pj => {
                const kmd = listKomoditas.find(k => k.id === pj.komoditasId || k.nama === pj.komoditasNama);
                const imgData = getKomoditasImageMap(pj.fotoPanen && pj.fotoPanen !== '🌾' && pj.fotoPanen !== '📦' ? pj.fotoPanen : (pj.komoditasNama || kmd?.nama || kmd?.gambar));
                return (
                  <tr key={pj.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 border border-gray-100 bg-gray-50 flex items-center justify-center">
                          <img src={imgData.url} alt={pj.komoditasNama} className="w-full h-full object-cover" />
                        </div>
                        <div>
                          <span className="font-semibold block text-gray-800">{pj.komoditasNama}</span>
                          {pj.tipePenjualan === 'PO_GUDANG' || pj.id.includes('_PO_') || pj.catatanPetani?.includes('PO GUDANG') ? (
                            <span className="inline-block mt-0.5 px-2 py-0.5 bg-blue-50 text-blue-700 text-[9px] font-bold rounded-md border border-blue-200">📦 Pemenuhan PO Gudang</span>
                          ) : (
                            <span className="inline-block mt-0.5 px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[9px] font-bold rounded-md border border-emerald-200">🧑‍🌾 Penjualan Mandiri</span>
                          )}
                        </div>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            {detail.fotoPanen && (detail.fotoPanen.startsWith('data:image/') || detail.fotoPanen.startsWith('http') || detail.fotoPanen.includes('.') || detail.fotoPanen.length > 50) ? (
              ...
              <div className="flex justify-center mb-4">
                <img 
                  src={getKomoditasImageMap(detail.komoditasNama || listKomoditas.find(k => k.id === detail.komoditasId)?.nama).url} 
                  alt="Sayuran" 
                  className="w-full max-h-48 object-cover rounded-2xl border border-gray-100 shadow-sm" 
                />
              </div>
            )}
