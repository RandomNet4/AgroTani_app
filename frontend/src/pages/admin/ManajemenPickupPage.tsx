...
import { kodePickup } from '../../utils/kode';

const ManajemenPickupPage: React.FC = () => {
  const { pickup: listPickup, pengajuanJual: listPengajuanJual, schedulePickup, updatePickupStatus } = useData();
  ...
  const pengajuanSiapPickup = listPengajuanJual.filter(
    pj => pj.status === 'approved' && pj.beratEstimasiKg >= 300 && !listPickup.some(pk => pk.pengajuanJualId === pj.id)
  );

  const handleSimpan = async () => {
    if (!form.pengajuanJualId || !form.tanggalPickup || !form.driverNama) return alert('Lengkapi form!');
    const pengajuan = listPengajuanJual.find(pj => pj.id === form.pengajuanJualId);
    if (!pengajuan) return;
    setLoading(true);
    const id = kodePickup(pengajuan.komoditasNama);
