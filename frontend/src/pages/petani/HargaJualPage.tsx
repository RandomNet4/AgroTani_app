...
const HargaJualPage: React.FC = () => {
  const { komoditas: listKomoditas, historiHarga: listHistoriHarga } = useData();
  ...
  const filtered = listKomoditas.filter(k => {
        ...
        <div className="space-y-3">
          {filtered.length > 0 ? (
            filtered.map(k => {
              ...
              const histori = listHistoriHarga.filter(h => h.komoditasId === k.id);
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 flex items-center gap-3">
                    {/* Image Photo */}
                    <div className="w-11 h-11 rounded-xl overflow-hidden shrink-0 border border-gray-100 shadow-xs">
                      <img src={getKomoditasImageMap(k.nama || k.gambar).url} alt={k.nama} className="w-full h-full object-cover" />
                    </div>
