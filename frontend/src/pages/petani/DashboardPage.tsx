...
const DashboardPage: React.FC = () => {
  const {
    ...
    tenderPetani: listTenderPetani
  } = useData();
  const navigate = useNavigate();

  const notifBelumDibaca = listNotifikasi.filter(n => !n.dibaca);
  const hargaTrending = listKomoditas.slice(0, 10);
  const pendingPO = listTenderPetani.filter(tp => tp.petaniId === currentUser?.id && tp.statusApproval === 'pending').length;

  // Perhitungan Data Kelompok Tani (Kepala Petani)
  const anggotaKelompok = allPetani.filter(p => p.kepalaPetaniId === currentUser?.id);
  const anggotaIds = anggotaKelompok.map(a => a.id);
  const lahanKelompok = listLahan.filter(l => anggotaIds.includes(l.petaniId));
  const tanamanKelompok = listTanamanAktif.filter(t => anggotaIds.includes(t.petaniId));
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-3 transition-all active:scale-[0.98] shadow-sm hover:shadow-md hover:border-emerald-100 group"
              >
                <div className="w-10 h-10 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-amber-100 transition-colors">
                  <Lightbulb size={18} />
                </div>
                <div className="text-left">
                  <p className="text-sm font-bold text-gray-800 leading-tight">Rekomendasi</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">Saran komoditas tani</p>
                </div>
              </button>
              ...
              >
                ...
                <div className="text-left">
                  <p className="text-sm font-bold text-gray-800 leading-tight">Jual Panen</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">Pengajuan panen kelompok</p>
                </div>
              </button>
            </div>
          </div>
              ...
              {hargaTrending.map(k => {
                ...
                return (
                  <div key={k.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-3">
                      <img src={getKomoditasImageMap(k.nama || k.gambar).url} alt={k.nama} className="w-9 h-9 rounded-lg object-cover border border-gray-100 shadow-xs shrink-0" />
                      <div>
