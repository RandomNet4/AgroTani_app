...
const TenderPage: React.FC = () => {
  const { tender: tenders, tenderPetani: tenderPetaniList, currentUser, refreshData } = useData();
  ...
  const tenderSaya = tenderPetaniList.filter(tp => tp.petaniId === petaniId);
        ...
        {tenderSaya.length > 0 && (
          <div className="bg-primary-50 border border-primary-100 rounded-2xl p-4">
            ...
            <div className="space-y-2">
              {tenderSaya.map(tp => {
                const tender = tenders.find(t => t.id === tp.tenderId);
                return (
                ...
                <div className="grid grid-cols-3 gap-2 mt-3">
                  <div className="bg-gray-50 rounded-xl p-2.5 text-center">
                    ...
                    <p className="font-bold text-sm text-gray-800 mt-0.5">
                      {tender.kebutuhanKg.toLocaleString('id-ID')} kg
                    </p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-2.5 text-center">
                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Terpenuhi</p>
                    <p className="font-bold text-sm text-gray-800 mt-0.5">
                      {tender.terpenuhinKg.toLocaleString('id-ID')} kg
                    </p>
                  </div>
