...
const JualPanenDetailPage: React.FC = () => {
  ...
  let pengajuan = pengajuanJual.find(p => p.id === id);
  if (!pengajuan && id && id.startsWith('PJB_PO_')) {
    const rawTpId = id.replace('PJB_PO_', '');
    const tp = (allTenderPetani || []).find(t => t.id === rawTpId);
    if (tp) {
      const tenderInfo = (allTender || []).find(t => t.id === tp.tenderId);
      ...
      pengajuan = {
        id,
        petaniId: tp.petaniId,
        petaniNama: tp.petaniNama,
        ...
        fotoPanen: '📦',
        status: tp.statusApproval === 'selesai' ? 'selesai' : (tp.statusApproval === 'dikirim' || tp.statusApproval === 'proses_timbang') ? 'proses_timbang' : tp.statusApproval === 'survey' ? 'survey' : tp.statusApproval === 'approved' ? 'approved' : 'pending',
  ...
  const pickup = allPickup.find(p => p.pengajuanJualId === pengajuan.id) ||
    allPickup.find(p => (p.petaniId === pengajuan.petaniId || (p.petaniNama && pengajuan.petaniNama && p.petaniNama.toLowerCase().trim() === pengajuan.petaniNama.toLowerCase().trim())) && (p.komoditasNama.toLowerCase().includes(pengajuan.komoditasNama.toLowerCase()) || pengajuan.komoditasNama.toLowerCase().includes(p.komoditasNama.toLowerCase())));

  const pembayaran = (pickup ? allPembayaran.find(p => p.pickupId === pickup.id) : null) ||
    allPembayaran.find(p => (p.petaniId === pengajuan.petaniId || (p.petaniNama && pengajuan.petaniNama && p.petaniNama.toLowerCase().trim() === pengajuan.petaniNama.toLowerCase().trim())) && (p.komoditasNama.toLowerCase().includes(pengajuan.komoditasNama.toLowerCase()) || pengajuan.komoditasNama.toLowerCase().includes(p.komoditasNama.toLowerCase())));
  ...
  const getStatusInfo = (status: string, beratEstimasiKg: number, hasQC: boolean) => {
    ...
    const info: Record<string, { label: string; desc: string; color: string }> = {
      ...
      'dikirim': { label: '📦 Dikirim ke Gudang', desc: 'Hasil panen sedang dalam perjalanan ke Gudang Cianjur. Menunggu penimbangan fisik petugas.', color: 'bg-indigo-50 border-indigo-200 text-indigo-800' },
      'pickup_dijadwalkan': { 
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-10">
      ...
      <div className="bg-gradient-to-b from-primary-700 to-primary-600 text-white px-5 pt-12 pb-6 rounded-b-[2rem] shadow-lg">
        ...
        {/* Komoditas Hero */}
        {(() => {
          const imgData = getKomoditasImageMap(pengajuan.fotoPanen && pengajuan.fotoPanen !== '🌾' && pengajuan.fotoPanen !== '📦' ? pengajuan.fotoPanen : (pengajuan.komoditasNama || komoditas?.nama || komoditas?.gambar));
          return (
            <div className="bg-white/15 backdrop-blur rounded-2xl p-4 border border-white/20 flex items-center gap-4">
              <div className="w-16 h-16 bg-white/20 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center border border-white/30">
                <img src={imgData.url} alt={pengajuan.komoditasNama} className="w-full h-full object-cover" />
              </div>
              <div>
            ...
            <div className="flex gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800 leading-relaxed">
                  {pengajuan.catatanAdmin.replace(/(?:PG|PO)-[A-Za-z0-9_-]+/g, (m) => formatPOCode(m, pengajuan.komoditasNama))}
                </p>
              </div>
            </div>
          </div>
