...
const FormTenderPage: React.FC = () => {
  const { tender: listTender, lahan: listLahan, currentUser } = useData();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const tenderId = searchParams.get('id') || '';
  const petaniId = currentUser?.id || '';

  const tender = listTender.find(t => t.id === tenderId);
  const lahanSaya = listLahan.filter(l => l.petaniId === petaniId);
  ...
  const handleSubmit = () => {
    ...
    navigate('/petani/pesanan-gudang');
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-5">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/petani/pesanan-gudang')} className="mt-3 text-primary-600 text-sm font-bold">
            ← Kembali
          </button>
        </div>
      </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3.5 border border-white/15">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div>
              ...
              <p className="font-bold text-xs mt-0.5">{tender.kebutuhanKg.toLocaleString('id-ID')} kg</p>
            </div>
            <div>
