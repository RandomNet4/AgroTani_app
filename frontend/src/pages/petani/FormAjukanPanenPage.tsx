...
import { formatRupiah, hitungHariMenuju } from '../../utils/formatters';
import { kodePenjualan } from '../../utils/kode';
import { getKomoditasImageMap } from '../../utils/cropHelpers';

const FormAjukanPanenPage: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { komoditas: listKomoditas, tanamanAktif: listTanamanAktif, lahan: listLahan, currentUser, addPengajuanJual } = useData();
  ...
  const tanamanSaya = listTanamanAktif.filter(t => t.petaniId === petaniId && t.statusVerifikasi === 'approved');
  ...
  const currentKomoditas = listKomoditas.find(k => k.id === currentKomoditasId);
  ...
  const currentLahan = listLahan.find(l => l.id === currentLahanId);
  const lahanSaya = useMemo(() => listLahan.filter(l => l.petaniId === petaniId), [listLahan, petaniId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const randomId = kodePenjualan(currentKomoditas?.nama);
            ...
            {sourceType ? (
              ...
              <div className="bg-white border-2 border-emerald-100 rounded-2xl p-4 shadow-sm relative overflow-hidden group">
                ...
                <div className="flex items-center gap-4 mt-3">
                  <div className="w-14 h-14 bg-gray-50 rounded-xl overflow-hidden shrink-0 border border-gray-100 shadow-inner flex items-center justify-center">
                    <img 
                      src={getKomoditasImageMap(sourceType === 'tanaman' ? selectedTanaman?.fotoTanaman || currentKomoditas?.nama : currentKomoditas?.nama || currentKomoditas?.gambar).url} 
                      alt={currentKomoditas?.nama || 'Sayuran'} 
          ...
          <div className="flex-1 p-5 overflow-y-auto bg-gray-50">
            {modalTab === 'tanaman' ? (
              <div className="space-y-3">
                {tanamanSaya.map(t => {
                  const komoditas = listKomoditas.find(k => k.id === t.komoditasId);
